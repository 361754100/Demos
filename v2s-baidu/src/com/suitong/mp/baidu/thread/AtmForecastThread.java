...
public class AtmForecastThread implements Runnable {
	...
	private List<Map<String,Object>> originList = null;
	...
	//用来记录上一次使用的是哪个ak，当ak变化的时候才用Logger输出,避免日志文件占用磁盘空间过大
//	private String akMarker = "";
	
	public AtmForecastThread(int pageNo, int pageSize){
	...
	public void run() {
		...
		logger.info("[AtmForecastThread] threadId->"+Thread.currentThread().getId()+" sleepTime->"+sleepTime+" ===begin===");
		
		this.originList = atmInfoService.getAtmInfoListByPage(pageNo, pageSize);
		logger.info("[AtmForecastThread] threadId->"+Thread.currentThread().getId()+"  originList->"+this.originList.size() );

		taskManager.setTaskRunning(true);
		if(this.originList!=null){
			try {
				initAtmForecastInfo();
				logger.info("[AtmForecastThread] threadId->"+Thread.currentThread().getId()+" ===finished===");
				taskManager.setTaskRunning(false);
				//如果允许再尝试的次数为0，则说明ak已被百度禁止，通过重启定时器再去尝试抓取数据
				if(continueTime == maxContinueTime){
					//先把原来的定时器关掉,再重启定时器任务
					taskManager.closeTimer();
					Thread.sleep(sleepTime);
					//再重启定时器任务
					AtmForecastTimer timerTask = new AtmForecastTimer( new AtmForecastThread( this.pageNo, this.pageSize ) );
					taskManager.beginAtmForecastTask(timerTask );
				}
			} catch (Exception e) {
				logger.error("[AtmForecastThread] threadId->"+Thread.currentThread().getId()+"  error->"+e.getMessage(),e);
				e.printStackTrace();
			} finally{
				taskManager.setTaskRunning(false);
			}
	 ...
	 * @throws Exception
	 */
	public Map<String,RouteMatrixJson> getAtmForecastInfoFromBaidu(List<Map<String, Object>> origins ) throws Exception{
		...
		if( operateType == AtmInfoService.OPERATE_TYPE_UPDATE ){
		...
		for(int i = 0; i< origins.size() ; i++){
			...
			for(int j = 0; j< destinations.size() ; j++){
				...
				if( (operateType == AtmInfoService.OPERATE_TYPE_ADD && isExist )
					...
					logger.info("[AtmForecastThread] threadId->"+Thread.currentThread().getId()+" origin["+i+"] operateType->"+operateType+" termStart->"+termStart+" termEnd->"+termEnd);
					continue;
				}
				matrixJson = null;
				if(tryNum == 0){
...
//		String url = "http://127.0.0.1/direction/v1/routematrix?output=json"
//				+ "&origins="+StringUtil.toString(origin.get("C_BD_Y"))+","+StringUtil.toString(origin.get("C_BD_X"))
//				+ "&destinations="+StringUtil.toString(destination.get("C_BD_Y"))+","+StringUtil.toString(destination.get("C_BD_X"))
//				+ "&ak="+akNow;
