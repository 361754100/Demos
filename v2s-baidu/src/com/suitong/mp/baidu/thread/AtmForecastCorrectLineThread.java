...
public class AtmForecastCorrectLineThread implements Runnable {
	...
	private AtmHttpReqManager reqManager = null;
	
	public AtmForecastCorrectLineThread(){
		...
		this.reqManager = AtmHttpReqManager.getInstance();
//		this.maxContinueTime = this.maxTryNum;
	}
	
	public void run() {
		tryNum = 5;
		if( taskManager.isCorrectLineRunning() ){
			logger.info("[AtmForecastHttpThread] threadId->"+Thread.currentThread().getId()+" isUpdatePartRunning->"+taskManager.isUpdatePartRunning() );
			return;
		}
		taskManager.setCorrectLineRunning(true);
		try {
			Map<String,String> taskParams = taskManager.getTaskParams();
			while( !reqManager.isCorrectLinesQueueEmpty() ){
				initAtmForecastInfo(taskParams);
			}
	...
	public void initAtmForecastInfo(Map<String,String> params) {
		...
		try {
			getAtmForecastInfoFromBaidu(params);
		} catch (Exception e) {
	 ...
	 * @throws Exception
	 */
	public void getAtmForecastInfoFromBaidu( Map<String,String> queryParams ) throws Exception{
		String lineSns = reqManager.getCorrectLineSns();
		
		List<Map<String,Object>> origins = atmInfoService.queryOriginListByLineSn(lineSns);
		if(origins == null || origins.size() == 0){
			return;
		}
		
		int[] correctPos = taskManager.getCorrectPos();
		int i = correctPos[0];
		...
		while( i < originSize ) {
			...
			destinations = atmInfoService.getAllDestinationList();
			...
			//获取还没采集的终点数据
			logger.info("[AtmForecastHttpThread] threadId->"+Thread.currentThread().getId()+" ["+i+"] origins->"+ originSize +" destinations->"+destinations.size() );
