 ...
 *
 */
public class AtmForecastInitThread implements Runnable {
	...
	public AtmForecastInitThread(int pageNo, int pageSize){
	...
	public void run() {
		...
		originList = atmInfoService.getModifyAtmInfoListByPage(this.pageNo, this.pageSize, modifyTime);
		...
		if(originList!=null){
			try {
				...
				if(continueTime == maxContinueTime){
					...
					//再重启定时器任务
					AtmForecastTimer timerTask = new AtmForecastTimer( new AtmForecastInitThread(this.pageNo, this.pageSize) );
	...
	public Map<String,RouteMatrixJson> getAtmForecastInfoFromBaidu(List<Map<String, Object>> origins ) throws Exception{
		...
		for(int i = 0; i< origins.size() ; i++){
			...
			String termEnd = StringUtil.toString(origin.get("C_TERMID"));
